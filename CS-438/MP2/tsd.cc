...
zhandle_t* zkh = nullptr;
std::string zk_node_path;
std::string server_dir;

void zk_watcher(zhandle_t* _zkh, int type, int state, const char* path, void* w_ctx)
{
  log(INFO, "ZK Event: type=" + std::to_string(type) + " state=" + std::to_string(state));
}
...
{
  std::ifstream file(server_dir + "/__" + username + "_following.txt");
  if (!file.is_open()) {
...
{
  std::ofstream file(server_dir + "/__" + username + "_following.txt", std::ios::trunc);  
...
class SNSServiceImpl final : public SNSService::Service {
  ...
  Status Connect(ServerContext* context, const Request* request, Reply* reply) override {
    ...
    log(INFO, "connected=" + std::to_string(c->connected) + 
    " last_hb=" + std::to_string(c->last_hb) + 
    " diff=" + std::to_string(difftime(time(0), c->last_hb)));

    // if (c.connected){
    if (c->connected && difftime(time(0), c->last_hb) < 2.0) {
    //    connection failed
...
Status Timeline(ServerContext* context,  ServerReaderWriter<Message, Message>* stream) override {
    ...
    //    Client c = getClient(u)
      ...
      if (i == 0) {
        ...
        //     stre
        std::ifstream tl_in(server_dir + "/__" + u + "_timeline.txt");
    ...
    // }
      ...
      // Append ffo to file u.txt
      std::ofstream u_out(server_dir + "/__" + u + ".txt", std::ios::app);
      if (u_out.is_open()) {
        u_out << ffo;
        u_out.close();
      }
    ...
    //       append ffo to the file f_following.txt    
      for (Client *f : c->client_followers) {
        ...
        std::ofstream f_out(server_dir + "/__" + f->username + "_timeline.txt", std::ios::app);
        if (f_out.is_open()) {
          f_out << ffo;
          f_out.close();
        }
...
int  StartZooKeeper(const std::string& port_no, 
                    const std::string& zk_host,
                    const std::string& cluster_id, 
                    const std::string& server_id)
{
  server_dir = "server_" + cluster_id + "_" + server_id;
  mkdir(server_dir.c_str(), 0777);
  log(INFO, "Setup directory " + server_dir);

  zoo_set_debug_level(ZOO_LOG_LEVEL_ERROR);

  if (!(zkh = zookeeper_init(zk_host.c_str(), zk_watcher, 30*1000, 0, 0, 0))) {
    log(ERROR, "Failed to connect to ZooKeeper at " + zk_host);
    return 1;
  }
  log(INFO, "Connected to ZooKeeper at " + zk_host);

  //TODO:ensure parent path
  zoo_create(zkh, "/services", nullptr, -1, &ZOO_OPEN_ACL_UNSAFE, 0, nullptr, 0);
  zoo_create(zkh, "/services/TinySNS-service", nullptr, -1, &ZOO_OPEN_ACL_UNSAFE, 0, nullptr, 0);

  std::string server_addr = "localhost:" + port_no;
  std::string node_path = "/services/TinySNS-service/" + server_dir + "_";

  char path_buffer[256];
  if (zoo_create(zkh, node_path.c_str(), server_addr.c_str(), server_addr.size(),
        &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL | ZOO_SEQUENCE, path_buffer, 256) != ZOK) {
    log(ERROR, "Failed to create zNode");
    return 1;
  }
  zk_node_path = path_buffer;
  //TODO: add a log here
  log(INFO, "Registered zNode: " + zk_node_path + " -> " + server_addr);
...
int main(int argc, char** argv) {
  ...
  std::string zk_host = ini["Server"]["ZKHOST"];
  std::string cluster_id = ini["Server"]["CLUSTER#"];
  ...
  if (StartZooKeeper(port, zk_host, cluster_id, server_id)) return 1;
