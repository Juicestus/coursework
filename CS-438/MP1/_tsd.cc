...
Client *getClient(const std::string &username, bool create = true) {
  ...
  for (Client *c : client_db) { // FIX: was == should be :
  ...
  while (std::getline(file, _username)) {
    if (!_username.empty()) {
      ...
      for (Client *c : client_following) {
        if (c->username ==
            _username) { // FIX: compare with _username not username
          already = true;
          break;
        }
      }
      if (!already) {
        ...
        followed->client_followers.push_back(
            this); // FIX: add to followed's followers
      }
    }
  }
...
void Client::saveFollowing() {
  std::ofstream file(
      username + "_following.txt",
      std::ios::trunc); // FIX: added file variable and fixed typo
...
class SNSServiceImpl final : public SNSService::Service {
  ...
  Status Follow(ServerContext *context, const Request *request,
    ...
    if (u1 == u2) {
      reply->set_msg("you have already joined"); // Match expected message
    ...
    for (Client *c : c1->client_following) { // FIX: was u1-> should be c1->
      if (c->username == u2) {
    ...
    c1->client_following.push_back(c2); // FIX: was u1/u2 should be c1/c2
    c2->client_followers.push_back(c1);

    c1->saveFollowing(); // FIX: was c1. should be c1->
  ...
  Status UnFollow(ServerContext *context, const Request *request,
    ...
    bool found = false;
    for (auto it = c1->client_following.begin();
         it != c1->client_following.end(); ++it) {
      if ((*it)->username == u2) {
    ...
    for (auto it = c2->client_followers.begin();
         it != c2->client_followers.end(); ++it) {
      if ((*it)->username == u1) {
  ...
  Status Connect(ServerContext *context, const Request *request,
    ...
    log(INFO, "Connect request from " + u);
  ...
  Status Timeline(ServerContext *context,
    ...
    while (stream->Read(&m)) {
      ...
      if (first) {
        ...
        // Read 20 latest messages from u_timeline.txt
    ...
    if (!first) { // Only if we actually entered timeline
  ...
  void RunServer(std::string port_no) {
    ...
    std::thread hb(heartbeat_checker);
    hb.detach();
  ...
  int main(int argc, char **argv) {
    ...
    if (argc != 2) {
      std::cerr
          << "Invalid Command Line Arguments: Usage .\\tsd <config.ini>\n";
      exit(0);
    }
