const projects = [
{
    ...
    <p>AMD Media Assistant reads articles scraped from the internet while providing a simple UI such that people with impaired vision can control it.
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    let inner = projectsArea.innerHTML;
    for (const project of projects) {
        inner += "<div class='text-only'><div class='text project-desc'>";
        inner += `<h3>${project.title}</h3>`;
        inner += project.description;
        if (project.code && !project.demo) {
            inner += `<p><a class="fancy-link" href="${project.code}">Source Code</a>`;
            // inner += `<a href="portfolio.html" class="twosigma-button">Source Code</a>`;
        }
        if (project.code && project.demo) {
            const demoText = project.customDemoText ? project.customDemoText : "Live Demo";
            inner += `<p><a class="fancy-link" href="${project.code}">Source Code</a>`;
            inner += ` | <a class="fancy-link" href="${project.demo}">${demoText}</a>`;
            // inner += `<div class="dual-twosigma-button-group align-left" style="width: 372px">
            //             <a href="${project.code}" class="dual-twosigma-button">
            //                 Source Code
            //             </a>
            //             <a href="${project.demo}" class="dual-twosigma-button">
            //                 ${demoText}
            //             </a>
            //         </div>`;
        }
